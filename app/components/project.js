...
function Project({data,index}) {
    ...
    return (
        <Link href={`/projects/${index + 1}`} passHref>
            <div className="w-72 h-72  md:w-96 md:h-96   p-1  rounded-xl flex-nowrap cursor-pointer   duration-500 hover:scale-105">
                ...
                <div className='w-full h-2/6 text-lg p-2  flex flex-col items-center justify-start'>
                    <h4 className='text-ellipsis text-pl font-sans text-secondary-light dark:text-secondary-dark'>{title}</h4>
                    <div className='flex flex-row w-56 z-20 [&>*]:duration-500 bg-gray-950/20 dark:bg-background-light px-4 py-2 white outline gap-12 rounded-3xl'>
                    {figma &&  
